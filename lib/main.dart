...
Future<void> main() async {
  ...
  await PokemonTcgApi.ensureCacheReady();
...
class _RecognizingScreenState extends State<RecognizingScreen> {
  ...
  Future<void> _runAll() async {
    ...
    try {
      ...
      _nameCtrl.text = _cleanOcrName(guess.name ?? '');
      _numCtrl.text = _cleanNumber(guess.number ?? '');
      ...
      if (name.isEmpty) {
        setState(() {
          _loading = false;
          _error = 'Could not detect a name. Please type it in.';
        });
        return;
      }

      // Try strict search first (name + number) if we have a number
      final api = PokemonTcgApi();
      List<PokemonCardResult> results = await api.searchCards(
        name: name,
        number: number.isEmpty ? null : number,
      );

      // If empty, broaden (name only)
      if (results.isEmpty) {
        results = await api.searchCards(name: name, number: null);
      }

      // Still empty => stay here and let user edit
      if (results.isEmpty) {
        setState(() {
          _loading = false;
          _error =
              'No matches found. The scan may have read the wrong text.\nEdit the fields and tap Search.';
        });
        return;
      }
      ...
      // If exactly one, go straight to Details
      if (results.length == 1) {
      ...
      Navigator.pushReplacement(
        ...
        MaterialPageRoute(
          builder: (_) => SearchResultsScreen(
            ...
            name: name,
            set: null,
            number: null, // keep broad so list is useful
          ),
        ),
      );
    } catch (e) {
